...
ui <- fluidPage(
  ...
  fluidRow(
    column(1),
    column(10,textOutput("instruction"),textOutput("instruction1")),
    column(1)
  ),
  ...
  fluidRow(
    column(1),
    column(10, "The app is created by Dr. Xi Cui at the Communication Department, College of Charleston. (Source codes can be found at githubxxx, CC-SA-NC, 2020)"),
    column(1)
  )
)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$help,{
    if (input$help%%2!=0){
      output$instruction <- renderText({"Prepare before class"})
      output$instruction1 <- renderText({"In class"})
